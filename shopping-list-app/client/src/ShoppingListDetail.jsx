...
import { PlusIcon, CheckIcon, XMarkIcon, ChatBubbleLeftEllipsisIcon, TrashIcon } from '@heroicons/react/24/solid';
...
function ShoppingListItem({ item, listId: parentListId, onToggle, onDelete, onUpdateComment, isOnline, queueItemAction }) {
    ...
    return (
        <li className="relative my-1 rounded overflow-hidden bg-background dark:bg-dark-background group/item shadow-sm border border-transparent hover:border-gray-200 dark:hover:border-gray-700">
...
function ShoppingListDetail({ list, items = [], currentUser, isOnline, queueItemAction, onBack, onDeleteList, onError, onTogglePrivacy, onUpdateComment }) {
    ...
    const handleOpenShareModal = () => { if (isOwner && list?._id && !list.tempId) { onError(null); setIsShareModalOpen(true); } else if (list?.tempId) { onError(t('share_unavailable_offline_list')); } else if (!isOwner) { onError(t('share_unavailable_not_owner')); } };
